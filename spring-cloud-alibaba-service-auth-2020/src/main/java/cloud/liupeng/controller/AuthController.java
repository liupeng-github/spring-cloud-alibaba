...
public class AuthController {
    ...
    public JsonResult index() {
        ...
        return JsonResult.success(HttpStatus.HTTP_OK, "认证服务", "端口号：" + port);
    }
    ...
    public JsonResult loginAuth(@PathVariable("username") String username) {
        ...
        if (users == null) {
            return JsonResult.success(HttpStatus.HTTP_NO_CONTENT, "登录认证失败，端口号：" + port);
        }
        return JsonResult.success(HttpStatus.HTTP_OK, "登录认证成功，端口号：" + port, users);
    }
}
