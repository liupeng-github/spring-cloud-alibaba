...
public class OrderController {
    ...
    public JsonResult index() {
        ...
        return JsonResult.success(HttpStatus.HTTP_OK, "订单服务", "端口号：" + port);
    }
    ...
    public JsonResult createOrder(@PathVariable("commodityCode") String commodityCode, @PathVariable("orderCount") Integer orderCount) {
        ...
        Integer storageTotal = dataLayerStorageService.getStorageTotalDatalayer(commodityCode);
        if (storageTotal == 0) {
            return JsonResult.success(HttpStatus.HTTP_NO_CONTENT, "商品库存不足，订单创建失败！");
        }

        log.info("减库存接口，商品编号：{}", commodityCode, "，订单量：{}", orderCount);
        int deduct = dataLayerStorageService.deductDatalayer(commodityCode, orderCount);
        if (deduct == 0) {
            return JsonResult.success(HttpStatus.HTTP_NO_CONTENT, "减库存失败，订单创建失败！");
        }
        ...
        if (createOrderDatalayer == 0 || createOrderDatalayer == null) {
            return JsonResult.success(HttpStatus.HTTP_NO_CONTENT, "订单创建失败！");
        }
        return JsonResult.success(HttpStatus.HTTP_OK, "成功创建订单", "订单：" + createOrderDatalayer);
    }
    ...
    public JsonResult getOrder(@PathVariable("orderId") Long orderId) {
        ...
        if (orderDatalayer == null) {
            return JsonResult.success(HttpStatus.HTTP_NO_CONTENT, "暂时未查询到该订单");
        }
        return JsonResult.success(HttpStatus.HTTP_OK, "查询订单接口，url：/order/getOrder，参数：" + orderId, orderDatalayer);
    }
}
